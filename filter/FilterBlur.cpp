...
void FilterBlur::blur1D(int beginIndex,
    ...
    for (int j = 1; j < num_step; j++){
        ...
        if(j+m_r >= num_step){
            next_r[ind] += vec_r[endIndex]*m_ker;
            next_g[ind] += vec_g[endIndex]*m_ker;
            next_b[ind] += vec_b[endIndex]*m_ker;
        }
        else{
            tmp_ind = shiftIndex(beginIndex, j+m_r, m_width, axis);
            next_r[ind] += vec_r[tmp_ind]*m_ker;
            next_g[ind] += vec_g[tmp_ind]*m_ker;
            next_b[ind] += vec_b[tmp_ind]*m_ker;
        }
    }
}
